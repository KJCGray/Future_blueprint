...
const workController = {
  postALLlanguage: (req, res) => {
    //回傳需要的前三個語言的聽說讀寫分別要什麼程度
    var arr = {
      job_L_class: req.session.job_L_class,
      area: req.session.area,
      job_type: req.session.job_type,
    };
      ...
      return new Promise((resolve, reject) => {
        languageDataModel.postALLlanguage(arr, bestName[index], (err, tmpresults) => {
          ...
          if (tmpresults && tmpresults.length > 0) {
            results.push({
              language: bestName[index],
              Label: tmpresults[0][bestName[index]],
              job_count: tmpresults[0]["CNT"],
            });
    ...
    skillDataModel.post(arr, (err, results) => {
      if (err) console.log(err);
        ...
        for (var i = 0; i < results.length; i++) {
          if (results[i]["job_skill"] == null || results[i]["job_skill"] == "")
            results[i]["job_skill"] = "不拘";
          if (results[i]["tool_expect"] == null || results[i]["tool_expect"] == "")
            results[i]["tool_expect"] = "不拘";
  ...
  postCertificate: (req, res) => {
    ...
    skillDataModel.certificateData(arr, (err, results) => {
      if (err) console.log(err);
        ...
        var cntMap = {};
        for (var i = 0; i < results.length; i++) {
          if (results[i]["certificates"] == null || results[i]["certificates"] == "")
            results[i]["certificates"] = "不拘";
